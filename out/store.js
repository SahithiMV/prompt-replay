...
class Store {
    ...
    // ---------- summaries ----------
    async writeSummary(eventId, summary) {
        await this.ensureDir();
        const sdir = this.summariesDir();
        if (!sdir)
            return;
        try {
            await vscode.workspace.fs.stat(sdir);
        }
        catch {
            await vscode.workspace.fs.createDirectory(sdir);
        }
    ...
    async readTrashIds() {
        ...
        try {
            ...
            const arr = JSON.parse(Buffer.from(buf).toString('utf8'));
            for (const id of arr)
                set.add(id);
        }
        catch { }
    ...
    async deleteEventPermanent(id) {
        ...
        if (snap) {
            try {
                await vscode.workspace.fs.delete(snap, { recursive: true, useTrash: true });
            }
            catch { }
        }
        // 3) summary
        ...
        if (sdir) {
            ...
            try {
                await vscode.workspace.fs.delete(sumFile, { useTrash: true });
            }
            catch { }
        }
        ...
        if (ex) {
            try {
                ...
                for (const [name, t] of entries) {
                    if (t !== vscode.FileType.File)
                        continue;
                    if (name.startsWith(`event-${id}`)) {
                        try {
                            await vscode.workspace.fs.delete(vscode.Uri.joinPath(ex, name), { useTrash: true });
                        }
                        catch { }
                    }
                }
